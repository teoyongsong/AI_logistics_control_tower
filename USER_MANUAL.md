...
|-------|------|------------------------|---------|------|----------------------------------------|
| **Region** | Select | `SG` · `MY` · `TH` · `PH` · `ID` · `VN` · `HK` · `TW` | `SG` | — | **Label** for the forecast service; does not change numeric heuristics unless you extend the model. |
...
| **Vehicle Type** | Select | `diesel_van` · `ev_van` · `hybrid_truck` | `diesel_van` | — | Affects **route fuel** and **CO₂ from the route tool**; **carbon** tab uses a separate per-vehicle emissions factor. |
| **Tool Retries** | Integer slider | `0` – `3` (count) | `1` | `1` | How many **retries** each tool call may use after a failure/timeout. Higher = more resilience, longer worst-case wait. |
| **Tool Timeout (s)** | Slider | `1.0` – `10.0` **seconds** | `3.0` | `0.5` | Max time per **tool** execution. Lower = fail fast; higher = allow slow steps. |
...
## 5) Top dashboard metrics (Plan + classic header)
...
|--------|----------------------------|------|---------------------|----------------|
| **Target ETA (min)** | About **tens of minutes** (depends on traffic × weather × fixed route geometry in code) | Minutes | **Route** tool: distance + traffic penalty × weather penalty → `estimated_eta_min` | **Lower is faster.** Rises with **high traffic** and **rain/storm**. Not tied to **Region** text. |
...
| **CO₂ Estimate (kg)** (10 Challenges header) | **≥ 0**, scales with **route distance** and **fuel** heuristic | kg CO₂ (approximate) | **Route** metrics: `co2_kg_estimate` ≈ `fuel_liters × 2.68` | **Lower is better** for the same trip; **vehicle** and **traffic** change fuel in the route model. |
...
## 6) “10 Challenges Demo” tab metrics
...
| **Distance (km)** | **≥ ~20** with default waypoints | km | Base distance from segment length + **waypoints** (hard-coded list length in orchestrator). |
...
| Metric | Range / values | Unit | Interpretation |
|--------|------------------|------|----------------|
| **Answer** | Text | — | Rule-based template from **query** / **tracking_id**. |
| **Escalate to human** | **Yes** / **No** | — | **Yes** if top retrieval confidence **below threshold** or query contains **complaint** (prototype rules). |
...
| **Explainability** | Base, urgency ×, demand ×, fuel surcharge | — | **Urgency:** standard **1.0**, express **1.25**, same_day **1.55**. **Demand multiplier** scales with **Demand Index** around 1.0. |
